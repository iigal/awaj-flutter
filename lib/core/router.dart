...
final gorouter = GoRouter(navigatorKey: rootNavigatorKey, initialLocation: "/", routes: routes);

final List<RouteBase> routes = [
  GoRoute(
      path: "/",
      redirect: (context, state) {
        final isLoggedIn = pocketBaseDB.authStore.isValid;
        final goingToRoot = state.fullPath == "/";

        if (isLoggedIn && goingToRoot) {
          return "/main/menu";
        }
        return null;
      },
      builder: (context, state) => const MainInitPage(),
      routes: [
        ...
        GoRoute(path: "main", builder: (context, state) => LoginPage(), routes: [
          ...
          StatefulShellRoute.indexedStack(
            redirect: (context, state) {
              if (!pocketBaseDB.authStore.isValid) {
                return "/main";
              }
              final getName = pocketBaseDB.authStore.record?.data['name'];
              if (getName == null || getName == '') {
                return "/main/user-details";
              }
              return null;
            },
